 ...
 */
int Zf(i_keccak256_init)(inner_keccak256_prng_ctx *sc)
{
    if (!sc)
        return -1;
 ...
 */
int Zf(i_keccak256_inject)(inner_keccak256_prng_ctx *sc, const uint8_t *in, size_t len)
{
    if (!sc || !in)
        return -1;
    if (sc->finalized)
 ...
 */
int Zf(i_keccak256_flip)(inner_keccak256_prng_ctx *sc)
{
    if (!sc)
        return -1;
    if (sc->finalized)
 ...
 */
int Zf(i_keccak256_extract)(inner_keccak256_prng_ctx *sc, uint8_t *out, size_t len)
{
    if (!sc || !out)
        return -1;
    if (!sc->finalized)
