...
// Mode selects the underlying deterministic backend used by optional APIs.
type Mode int
...
func GenerateKey(seed []byte) (PublicKey, PrivateKey, error) {
	return GenerateKeyWithMode(seed, ModeSHAKE)
}

// GenerateKeyWithMode generates a public/private key pair using the selected backend.
// ModeSHAKE preserves the existing behavior. Keccak mode is currently only
// implemented for hash-to-point selection; key generation still routes through
// the existing SHAKE-based implementation.
func GenerateKeyWithMode(seed []byte, mode Mode) (PublicKey, PrivateKey, error) {
	_ = mode
...
func (sk *PrivateKey) SignCompressed(msg []byte) (CompressedSignature, error) {
	return sk.SignCompressedWithMode(msg, ModeSHAKE)
}

// SignCompressedWithMode signs the message using the selected backend.
func (sk *PrivateKey) SignCompressedWithMode(msg []byte, mode Mode) (CompressedSignature, error) {
	var sigLen C.size_t
...
// SignCompressedKeccak signs the message using the Keccak-backed deterministic mode.
func (sk *PrivateKey) SignCompressedKeccak(msg []byte) (CompressedSignature, error) {
	return sk.SignCompressedWithMode(msg, ModeKeccak)
}
...
func (pk *PublicKey) Verify(signature CompressedSignature, msg []byte) error {
	return pk.VerifyWithMode(signature, msg, ModeSHAKE)
}

// VerifyWithMode verifies the signature using the selected backend.
func (pk *PublicKey) VerifyWithMode(signature CompressedSignature, msg []byte, mode Mode) error {
	if len(signature) == 0 {
...
// VerifyKeccak verifies the signature using the Keccak-backed deterministic mode.
func (pk *PublicKey) VerifyKeccak(signature CompressedSignature, msg []byte) error {
	return pk.VerifyWithMode(signature, msg, ModeKeccak)
}
...
func HashToPointCoefficients(msg []byte, saltVersion byte) (c [N]uint16) {
	return HashToPointCoefficientsWithMode(msg, saltVersion, ModeSHAKE)
}

// HashToPointCoefficientsWithMode hashes the message using the selected backend.
