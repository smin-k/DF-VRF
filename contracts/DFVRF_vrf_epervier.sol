...
contract ZKNOX_vrf_epervier {
    ...
    /// @param transcript the VRF transcript to be signed
    ...
    function recover(bytes memory transcript, bytes memory salt, uint256[] memory cs1, uint256[] memory cs2, uint256 hint)
        public
        pure
        returns (address result)
    {
        ...
        assembly {
            for { let offset := 32 } gt(16384, offset) { offset := add(offset, 32) } {
                let temp := mulmod(mload(add(s2_inverse_ntt, sub(16416, offset))), mload(add(s2, sub(16416, offset))), q)
